...
namespace T2IN1_Wukong
{
    internal class JungleClear
    {
        public static void Execute()
        {
            ...
            var JungleMinionW = ObjectManager.Get<Obj_AI_Minion>().FirstOrDefault(njmw => njmw.IsMonster && njmw.IsValidTarget(W.Range = 150));
            if (JungleClearMenu["W"].Cast<CheckBox>().CurrentValue)
                if (W.IsReady() && W.IsLearned)
                    W.TryToCast(JungleMinionW, JungleClearMenu);

            var JungleMinionE = ObjectManager.Get<Obj_AI_Minion>().FirstOrDefault(njme => njme.IsMonster && njme.IsValidTarget(E.Range));
        ...
        public static void Game_OnTick(EventArgs agrs)
        {
            ...
            if (IMinionC != null && IMinionC.IsValid && Important)
                if (IMinionC.Health <= Player.Instance.GetSummonerSpellDamage(IMinionC, DamageLibrary.SummonerSpells.Smite))
                    Smite.Cast(IMinionC);

            if (RedBuff != null && RedBuff.IsValid && JungleClearMenu["JRed&Blue"].Cast<CheckBox>().CurrentValue)
                if (RedBuff.Health <= Player.Instance.GetSummonerSpellDamage(IMinionC, DamageLibrary.SummonerSpells.Smite))
                    Smite.Cast(RedBuff);
